    ...
    try:
        async with httpx.AsyncClient(timeout=_OLLAMA_TIMEOUT) as client:
            payload: dict = {
                ...
                "messages": messages,
                "stream": False,
                # Ollama defaults num_ctx to ~4096 when neither the request nor
                # the Modelfile sets it. The tool schemas alone are ~13.9k
                # tokens, so leaving this unset silently truncated the prompt
                # and hid most tools from the model.
...
def _notify_vram_broker() -> None:
    # Ollama just served a completion, so its model is resident again even if
    # the broker evicted it earlier — keep the broker's view in sync.
    try:
        from voice.vram_broker import get_vram_broker
        get_vram_broker().notify_active("ollama")
    except Exception:
        pass
