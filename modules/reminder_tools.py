...
def set_reminder(message: str, minutes: int) -> str:
    ...
    from core.watchdog import reactivation_warning
    fire_at = datetime.now(timezone.utc) + timedelta(minutes=minutes)
    get_memory_store().add_reminder(message, fire_at.isoformat())
    warn = reactivation_warning("reminder_loop", "A reminder was created")
    return f"Reminder set: '{message}' in {minutes} minute(s)." + warn
...
def set_timer(minutes: int = 0, seconds: int = 0, label: str = "") -> str:
    ...
    warn = reactivation_warning("reminder_loop", "A reminder was created")
    return f"Timer set for {duration}{': ' + label if label else ''}." + warn
...
@tool(description="Clear all pending reminders at once. Use when the user asks to clear, "
      "delete, or cancel all their reminders. Timers are not affected — use cancel_timer for those.")
def clear_all_reminders() -> str:
    from agents.memory_store import get_memory_store
    count = get_memory_store().clear_pending_reminders()
    if not count:
        return "No pending reminders to clear."
