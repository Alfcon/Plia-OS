...
@tool(
    "Triage the inbox: classify recent emails by urgency (high/normal/low), category, and "
    "spam-likelihood, with a one-line summary each and a suggested reply draft for urgent "
    "ones. Read-only — nothing is moved, labelled, marked, deleted, or sent. Optionally "
    "specify an account name."
)
def triage_inbox(account: str = "", max_items: int = 10) -> str:
    acc = _resolve(account)
    if acc is None:
        return _NO_ACCOUNTS
    try:
        ...
        items = [
            {"from": m.from_ or "", "subject": m.subject or "", "body": _get_body(m)}
            for m in msgs
        ]
        results = triage_messages(items)
        ranked = sorted(
            zip(msgs, results),
            key=lambda pair: _URGENCY_RANK.get(pair[1]["urgency"], 1),
        )
        parts = [_SEP]
        for msg, r in ranked:
            spam = "spam" if r["spam"] else "not spam"
            parts.append(f"[{r['urgency'].upper()}] {r['category']} · {spam}")
            parts.append(f"From:    {_parse_from(msg.from_ or '')}")
            parts.append(f"Subject: {msg.subject or '(no subject)'}")
            parts.append(f"Summary: {r['summary']}")
            if r["draft"]:
                parts.append(f"Draft:   {r['draft']}")
            parts.append(_SEP)
        return "\n".join(parts)
    except RuntimeError as exc:
        return f"Email authentication failed: {exc}"
