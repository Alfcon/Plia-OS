...
def add_research_site(
    ...
    category: Literal["academic", "dev", "general"] = "general",
) -> str:
...
def remove_research_site(slug: str) -> str:
    ...
    from core.browser_session import clear_session
    if remove_site(slug):
...
def remove_site_credentials(site_slug: str) -> str:
    ...
    if remove_credentials(site_slug):
        clear_session(site_slug)
        return f"Credentials for '{site_slug}' removed."
    return f"No credentials found for '{site_slug}'."


@tool(
    "Open a browser to sign in to a login-required research site. "
    "The site must already have credentials stored via set_site_credentials. "
    "After you finish signing in, run finish_login with the same slug to save the session."
)
async def login_site(slug: str) -> str:
    ...
    site = get_site(slug)
    if site is None:
        return f"No site with slug '{slug}' found. Run list_research_sites to see available sites."
    if not site.get("requires_login"):
        return f"Site '{slug}' does not require login."
    creds = get_credentials(slug)
    if creds is None:
        return f"No credentials stored for '{slug}'. Run set_site_credentials('{slug}', username, password) first."
    return await browser_session.start_login(slug, site["url"], creds["username"], creds["password"])


@tool(
    "Save the browser session after you have finished signing in to a research site with login_site. "
    "Run this once sign-in is complete in the opened browser window."
)
async def finish_login(slug: str) -> str:
    from core import browser_session
    return await browser_session.finish_login(slug)
    ...
    async def _search_one(slug: str) -> tuple[str, object]:
        ...
        if site["requires_login"]:
            ...
            if not has_credentials(slug):
                return site_name, (
                    f"[LOGIN REQUIRED — use set_site_credentials('{slug}', username, password), then login_site('{slug}')]"
                )
            if not browser_session.has_session(slug):
                return site_name, f"[LOGIN REQUIRED — run login_site('{slug}') to sign in once]"
            html, ok = await browser_session.authed_fetch(slug, url)
            if not ok:
                return site_name, f"[SESSION EXPIRED — run login_site('{slug}') to sign in again]"
            parser = _SITE_PARSERS.get(slug)
            snippets = parser(html, url) if parser else _extract_snippets(html, url)
