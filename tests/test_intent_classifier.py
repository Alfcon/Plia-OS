...
def test_courtesy_routes_to_respond():
    # "Thank you." previously fell below threshold and punted to the LLM router,
    # which then misclassified it using topic-heavy context (e.g. prior weather).
    intent, conf = classify_intent("thank you")
    assert conf >= THRESHOLD
    assert intent == "respond"


def test_greeting_classified_respond():
