...
async def test_pipeline_emits_status_events():
    ...
    speech = np.ones(1280, dtype=np.float32) * 0.1 * 32768.0  # int16-scale; rms 0.1 > floor
...
async def test_tts_reaches_playback():
    """The speak phase must actually reach sd.play.

    Regression: `_process_loop` called `_gc()`, a name bound only inside
    `start`/`_speak_announcement`. The resulting NameError was swallowed by
    `except Exception: logger.exception("TTS playback error")`, so every turn
    "completed" while Plia never made a sound. Status-event assertions cannot
    catch this — only asserting playback can.
    """
    pipeline = _make_pipeline(wake_detects_on=0)
    pipeline._conversation = [{"role": "system", "content": "sys"}]
    ...
    silence = np.zeros(1280, dtype=np.float32)
    speech = np.ones(1280, dtype=np.float32) * 0.1 * 32768.0  # int16-scale; rms 0.1 > floor
    chunks = [np.ones(1280, dtype=np.float32) * 0.9] + [speech] * 5 + [silence] * 25

    audio_q: asyncio.Queue = asyncio.Queue()
    for c in chunks:
        await audio_q.put(c)

    with patch("voice.pipeline.run_turn", new=AsyncMock(return_value=("Lights on!", []))), \
         patch("voice.pipeline.sd") as mock_sd:
        mock_sd.play = MagicMock()
        await pipeline._process_loop(audio_q, max_iterations=1)

    pipeline._tts.synthesise.assert_called_once()
