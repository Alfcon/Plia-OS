...
async def test_get_history_returns_messages(app):
    ...
    with patch("agents.chat_history.get_recent_with_id", return_value=messages):
...
async def test_get_history_empty(app):
    with patch("agents.chat_history.get_recent_with_id", return_value=[]):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
            r = await ac.get("/api/history")
    assert r.status_code == 200
    assert r.json() == []
...
async def test_get_history_passes_n_param(app):
    with patch("agents.chat_history.get_recent_with_id", return_value=[]) as mock_get:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
            await ac.get("/api/history?n=25")
    mock_get.assert_called_once_with(25, True)
