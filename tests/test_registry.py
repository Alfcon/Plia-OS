...
def test_literal_param_emits_enum_and_default():
    @tool(description="ocr an image")
    def ocr(path: str, engine: Literal["auto", "tesseract", "vision"] = "auto") -> str:
        return engine

    prop = get_tool_schemas()[0]["function"]["parameters"]["properties"]["engine"]
    assert prop["type"] == "string"
    assert prop["enum"] == ["auto", "tesseract", "vision"]
    assert prop["default"] == "auto"
    # enum param has a default → optional, not required
    assert "engine" not in get_tool_schemas()[0]["function"]["parameters"]["required"]


def test_literal_with_blank_choice_preserves_empty_default():
    @tool(description="propose a command")
    def propose(cmd: str, risk: Literal["", "low", "medium", "high"] = "") -> str:
        return risk

    prop = get_tool_schemas()[0]["function"]["parameters"]["properties"]["risk"]
