...
def test_apply_preset_drops_old_semantics_silence_timeout(tmp_path):
    # A preset saved before the silence_timeout_seconds semantics change
    # (identified by the retired silence_chunks_threshold key) would re-inject
    # the old whole-utterance-cap value into the new trailing-silence meaning.
    import json as _json
    from core.config import get_config, reset_config, update_config
    import core.preset_store as ps
    ...
    try:
        ...
        ps._PRESETS_FILE.write_text(_json.dumps({"old": old_preset}))
        update_config(silence_timeout_seconds=1.0)
        assert ps.apply_preset("old") is True
        cfg = get_config()
        assert cfg.silence_timeout_seconds == 1.0   # untouched by the preset
        assert cfg.ollama_model == "phi3"           # rest of preset applied
    finally:
        ps._PRESETS_FILE = original
        reset_config()
