...
def apply_preset(name: str) -> bool:
    ...
    if name not in data:
        return False
    preset = data[name]
    if "silence_chunks_threshold" in preset:
        # Preset predates the silence_timeout_seconds semantics change
        # (whole-utterance cap -> trailing-silence window); the saved value
        # would silently re-inject the old meaning. Same marker as
        # core.config._load_persisted.
        preset = {k: v for k, v in preset.items() if k != "silence_timeout_seconds"}
