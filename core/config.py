...
class PliaConfig:
    ...
    # Pipeline
    # Silence that ends a turn. Was previously a hard cap on the whole
    # utterance despite the name, which truncated anything longer.
    silence_timeout_seconds: float = 0.8
    # Runaway guard: stop listening even if the speaker never pauses.
    max_utterance_seconds: float = 15.0
    # How long after a wake word to wait for speech before re-arming.
    # A false wake never trips the silence exit (that requires speech first),
    # so without this bail the mic would stay held for max_utterance_seconds.
    prespeech_bail_seconds: float = 2.0
    ...
    # Ollama context window. Ollama silently defaults to ~4096 when unset,
    # which truncates the ~13.9k-token tool schema payload and leaves the
    # model unable to see most of its own tools.
    ollama_num_ctx: int = 16384

    # Adaptation / personalization (default off — see
    # docs/superpowers/specs/2026-07-15-adaptation-personalization-design.md)
    adaptation_enabled: bool = False
    adaptation_route_examples: int = 3
    adaptation_style_examples: int = 3
    adaptation_direct_threshold: float = 0.95
    adaptation_rephrase_threshold: float = 0.85
    ...
    # Watchdog — persisted task lifecycle states, name -> "paused" | "stopped"
    watchdog_task_states: dict = field(default_factory=dict)
    ...
    # User-added news search topics persisted across restarts
    news_custom_topics: list = field(default_factory=list)
    news_saved_topics: list = field(default_factory=list)

    # RSS feed settings persisted across restarts
    rss_custom_urls: list = field(default_factory=list)
    rss_saved_urls: list = field(default_factory=list)
...
# Inclusive (min, max) bounds for numeric fields where an out-of-range value
# breaks the voice pipeline outright (max_utterance_seconds <= 0 means the
# listen loop never runs; silence_timeout_seconds = 0 cuts every turn at the
# first quiet chunk). Bounds match the dashboard controls so a persisted value
# can never exceed what the UI can represent.
_RANGE_CONSTRAINTS: dict[str, tuple[float, float]] = {
    "silence_timeout_seconds": (0.2, 3.0),
    "max_utterance_seconds": (3.0, 60.0),
    "prespeech_bail_seconds": (0.5, 10.0),
}


def _in_range(key: str, value) -> bool:
    bounds = _RANGE_CONSTRAINTS.get(key)
    if bounds is None:
        return True
...
def _load_persisted(config: PliaConfig) -> None:
    ...
    try:
        ...
        if "silence_chunks_threshold" in data:
            # File predates the silence_timeout_seconds semantics change
            # (whole-utterance cap -> trailing-silence window), so its persisted
            # value is meaningless under the new meaning. Self-limiting: the
            # first save drops silence_chunks_threshold and this never fires again.
            data.pop("silence_timeout_seconds", None)
        for key, value in data.items():
            ...
            if not _in_range(key, value):
                logger.warning(
                    "Ignoring out-of-range persisted value %r for %r; allowed: %s",
                    value, key, _RANGE_CONSTRAINTS[key],
                )
                continue
...
def update_config(**kwargs) -> PliaConfig:
    ...
    for key, value in kwargs.items():
        ...
        if not _in_range(key, value):
            raise ValueError(
                f"Value {value!r} for {key!r} out of range; "
                f"allowed: {_RANGE_CONSTRAINTS[key]}"
            )
