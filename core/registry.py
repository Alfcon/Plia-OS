...
def _build_parameters(fn: Callable) -> dict:
    ...
    for name, param in sig.parameters.items():
        ...
        if get_origin(py_type) is Literal:
            # Literal["a", "b"] → constrained enum (dropdown in UI + LLM schema)
            choices = list(get_args(py_type))
            base = type(choices[0]) if choices else str
            prop = {"type": _TYPE_MAP.get(base, "string"), "enum": choices}
        else:
...
def _module_of(fn: Callable) -> str:
    # A module imported early via `from modules.X import ...` registers its
    # tools while _LOADING_MODULE still names the *importing* module — derive
    # attribution from the function itself so disabled_modules stays correct.
    mod = getattr(fn, "__module__", "") or ""
    if mod in ("__main__", "builtins"):
        return _LOADING_MODULE
    return mod.removeprefix("modules.") or _LOADING_MODULE
