...
from core.watchdog import watchdog_gate, is_paused
...
class ObserverService:
    ...
    async def start(self) -> None:
        ...
        self._tasks = [
            asyncio.create_task(self._screen_loop(), name="observer_screen_loop"),
            asyncio.create_task(self._focus_loop(), name="observer_focus_loop"),
            asyncio.create_task(self._key_loop(), name="observer_key_loop"),
            asyncio.create_task(self._profile_loop(), name="observer_profile_loop"),
        ]
    ...
    async def _key_loop(self) -> None:
        ...
        try:
            async for event in dev.async_read_loop():
                if is_paused("observer"):
                    # Discard events read while paused instead of buffering
                    # them behind the gate — otherwise everything typed
                    # during the pause gets processed in a burst on resume.
                    continue
                try:
    ...
    async def _run_profile_once(self) -> None:
        try:
            ...
            if cfg.fallback_provider:
                # Warn once per process — this fires every poll interval and
                # the condition only changes when the user edits config.
                if not getattr(self, "_profile_skip_warned", False):
                    self._profile_skip_warned = True
                    logger.warning(
