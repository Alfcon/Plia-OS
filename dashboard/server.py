...
from core import events, registry, pipeline_registry, watchdog as _watchdog
...
@router.get("/api/hf/quants")
async def hf_quants_route(repo: str):
    """List GGUF quantizations for a Hugging Face repo, with hardware fit.

    Falls back to the most-downloaded GGUF conversion (HF model-tree
    lineage) when the repo itself has no GGUF files.
    """
    import httpx
    from core.hf_quants import extract_quants, parse_hf_repo
    from core.model_catalog import current_hardware

    repo_id = parse_hf_repo(repo)
    if not repo_id:
        raise HTTPException(status_code=400,
                            detail="not a Hugging Face repo URL or id")

    async def _tree(client: "httpx.AsyncClient", rid: str) -> list:
        r = await client.get(
            f"https://huggingface.co/api/models/{rid}/tree/main")
        if r.status_code in (401, 403):
            raise HTTPException(
                status_code=403,
                detail=(f"{rid} is gated on Hugging Face — accept its terms "
                        "in a browser, or pull a public GGUF conversion"))
        if r.status_code == 404:
            raise HTTPException(status_code=404,
                                detail=f"{rid} not found on Hugging Face")
        r.raise_for_status()
        data = r.json()
        return data if isinstance(data, list) else []

    def _has_gguf(files: list) -> bool:
        return any(isinstance(f, dict) and isinstance(f.get("path"), str)
                   and f["path"].lower().endswith(".gguf") for f in files)

    source_repo, alternatives = repo_id, []
    try:
        async with httpx.AsyncClient(timeout=10.0,
                                     follow_redirects=True) as client:
            files = await _tree(client, repo_id)
            if not _has_gguf(files):
                r = await client.get(
                    "https://huggingface.co/api/models",
                    params={"filter": f"base_model:quantized:{repo_id},gguf",
                            "sort": "downloads", "limit": 6},
                )
                r.raise_for_status()
                data = r.json()
                candidates = [
                    m["id"] for m in (data if isinstance(data, list) else [])
                    if isinstance(m, dict) and isinstance(m.get("id"), str)]
                if not candidates:
                    return {"repo": repo_id, "source_repo": repo_id,
                            "alternatives": [], "quants": []}
                source_repo, alternatives = candidates[0], candidates[1:]
                files = await _tree(client, source_repo)
    except HTTPException:
        raise
    except (httpx.HTTPError, ValueError) as exc:
        logger.warning("HF quant lookup failed: %s", exc)
        if isinstance(exc, httpx.HTTPStatusError) and exc.response.status_code == 429:
            raise HTTPException(
                status_code=502,
                detail="rate limited by Hugging Face — try again shortly")
        raise HTTPException(status_code=502,
                            detail=f"Hugging Face unreachable: {exc}")

    vram, ram = current_hardware()
    return {"repo": repo_id, "source_repo": source_repo,
            "alternatives": alternatives,
            "quants": extract_quants(files, vram, ram)}


async def _run_one(model: str, prompt: str) -> dict:
    """Run one prompt through one model via Ollama. Never raises."""
    import time
    import httpx
    cfg = get_config()
    ...
    try:
        ...
        return {"model": model, "output": output, "ms": ms, "tokens": tokens}
    except Exception as exc:
        ms = int((time.perf_counter() - start) * 1000)
        return {"model": model, "output": f"Error: {exc}", "ms": ms, "tokens": 0}
...
async def compare_models(body: dict):
    ...
    if not prompt or not model_a or not model_b:
        raise HTTPException(status_code=400, detail="prompt, model_a, and model_b are required")
    a, b = await asyncio.gather(_run_one(model_a, prompt), _run_one(model_b, prompt))
    return {"a": a, "b": b}


@router.get("/api/models/recommend")
async def recommend_models_route(want: str = ""):
    import httpx
    from core.model_catalog import current_hardware, rank_catalog, merge_installed
    vram, ram = current_hardware()
    models = rank_catalog(vram, ram)
    installed: list = []
    try:
        async with httpx.AsyncClient(timeout=2.0) as client:
            r = await client.get(f"{get_config().ollama_url}/api/tags")
            r.raise_for_status()
            installed = r.json().get("models") or []
            if not isinstance(installed, list):
                installed = []
    except Exception:
        pass  # Ollama down/unreachable → catalog only
    models = merge_installed(models, installed, vram, ram)
    want = (want or "").strip().lower()
    if want:
        models = [m for m in models if want in m["tags"]]
    return {
        "hardware": {"vram_gb": round(vram, 2), "ram_gb": round(ram, 2)},
        "current": get_config().ollama_model,
        "models": models,
    }


@router.get("/api/models/tags")
async def model_catalog_tags():
    from core.model_catalog import CATALOG
    tags = sorted({t for m in CATALOG for t in m.tags})
    return {"tags": tags}
...
async def post_config(updates: dict):
    ...
    if config.tts_engine != old_engine:
        ...
        if svc is not None:
            ...
            asyncio.create_task(_switch_and_broadcast(), name="tts_engine_switch")
...
async def get_history(n: int = 100):
    from agents.chat_history import get_recent_with_id
    # Include message IDs so the UI can perform per-message operations.
    return await asyncio.to_thread(get_recent_with_id, n, True)


@router.delete("/api/history/{message_id}")
async def delete_history_message(message_id: int):
    from agents.chat_history import delete_message
    deleted = await asyncio.to_thread(delete_message, message_id)
    if not deleted:
...
@router.post("/api/adaptation/rate")
async def rate_message(req: RateRequest):
    from agents.chat_history import get_message_pair
    from agents.adaptation import get_adaptation_store

    pair = await asyncio.to_thread(get_message_pair, req.message_id)
    if pair is None:
        raise HTTPException(status_code=404, detail="Assistant message not found")
    user_msg, assistant_msg = pair
    store = get_adaptation_store()
    # rating=-1 is stored but never indexed — it can exclude, never teach.
    await asyncio.to_thread(
        store.add_style_exemplar,
        user_msg["content"], assistant_msg["content"], req.rating, "thumb",
    )
    return {"status": "rated", "id": req.message_id, "rating": req.rating}


@router.get("/api/adaptation/exemplars")
async def list_adaptation_exemplars(limit: int = 200):
    from agents.adaptation import get_adaptation_store
    store = get_adaptation_store()
    return {"exemplars": await asyncio.to_thread(store.list_exemplars, limit)}


@router.delete("/api/adaptation/exemplars/{exemplar_id}")
async def delete_adaptation_exemplar(exemplar_id: int):
    from agents.adaptation import get_adaptation_store
    store = get_adaptation_store()
    deleted = await asyncio.to_thread(store.delete_exemplar, exemplar_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Exemplar not found")
    return {"status": "deleted", "id": exemplar_id}


@router.delete("/api/adaptation/exemplars")
async def clear_adaptation_exemplars():
    from agents.adaptation import get_adaptation_store
    store = get_adaptation_store()
...
async def shutdown():
    ...
    asyncio.create_task(_do(), name="shutdown_delay")
...
async def pipeline_stop():
    ...
    # In-memory only: the watchdog view should reflect reality, but we must
    # not persist "stopped" here — that would block the voice_pipeline
    # watchdog task from spawning again at next boot even though this is a
    # transient pipeline-panel action, not a watchdog stop.
    _watchdog.mark_stopped_transient("voice_pipeline")
...
async def pipeline_start():
    ...
    new_task = asyncio.create_task(start_pipeline(), name="pipeline_start")
    pipeline_registry.set_task(new_task)
    pipeline_registry.set_state("starting")
    _watchdog.set_task("voice_pipeline", new_task)
    _watchdog.clear_state("voice_pipeline")
...
_OS_PERMISSION_GROUPS = [
    {
        ...
        "description": "Allows tools to change MAC addresses (ip link set) and local IP addresses (ip addr add/del) using sudo.",
        "tools": ["randomize_mac", "set_mac", "restore_mac", "randomize_local_ip", "set_local_ip", "restore_ip"],
...
async def run_vram_sampler() -> None:
    ...
    while True:
        await _watchdog.watchdog_gate("vram_sampler")
        try:
...
def _gpu_processes() -> dict:
    """List GPU processes via nvidia-smi. Empty result on any failure —
    absence of NVIDIA tooling is a normal condition, not an error."""
    try:
        proc = subprocess.run(
            ["nvidia-smi", "-q", "-x"],
        ...
        if proc.returncode != 0:
            return {"processes": [], "total_mib": 0}
        root = _ET.fromstring(proc.stdout)
    except Exception:
        return {"processes": [], "total_mib": 0}
    ...
    for info in root.iter("process_info"):
        try:
            pid = int(info.findtext("pid", "0"))
        except ValueError:
            continue
        mem_text = (info.findtext("used_memory") or "").strip()
        try:
            used_mib = int(mem_text.split()[0])
        except (ValueError, IndexError):
            used_mib = 0
        processes.append({
            "pid": pid,
            "name": info.findtext("process_name", "?") or "?",
            "used_mib": used_mib,
            "is_self": pid == self_pid,
        })
    processes.sort(key=lambda p: p["used_mib"], reverse=True)
    return {"processes": processes, "total_mib": sum(p["used_mib"] for p in processes)}


@router.get("/api/vram/processes")
async def vram_processes():
    return await asyncio.to_thread(_gpu_processes)


def _ram_processes() -> dict:
    """Top RAM-consuming process groups via psutil. Zeroed result on any
    top-level failure — the RAM popup is informational, never an error."""
    empty = {"processes": [], "more_count": 0, "more_mib": 0, "used_mib": 0, "total_mib": 0}
    try:
        self_pid = os.getpid()
        ...
        for proc in psutil.process_iter(["pid", "name", "memory_info"]):
            try:
                ...
                g = groups.setdefault(name, {"name": name, "count": 0, "rss": 0, "is_self": False})
                g["count"] += 1
                g["rss"] += rss
                if info.get("pid") == self_pid:
                    g["is_self"] = True
            except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
                # Defensive: real process_iter(attrs=...) filters dead procs
                # internally; this exception should not fire in normal operation.
                continue
        ranked = sorted(groups.values(), key=lambda g: g["rss"], reverse=True)
        top, rest = ranked[:10], ranked[10:]
        vm = psutil.virtual_memory()
        return {
            "processes": [
                {"name": g["name"], "count": g["count"], "rss_mib": g["rss"] // (1024 * 1024), "is_self": g["is_self"]}
                for g in top
            ],
            "more_count": len(rest),
            "more_mib": sum(g["rss"] for g in rest) // (1024 * 1024),
...
@router.get("/api/ram/processes")
async def ram_processes():
    return await asyncio.to_thread(_ram_processes)


def _cpu_processes() -> dict:
    """Top CPU-consuming process groups via two-pass psutil sampling.
    Zeroed result on any top-level failure — the CPU popup is
    informational, never an error."""
    empty = {"processes": [], "more_count": 0, "more_percent": 0.0, "total_percent": 0.0}
    try:
        self_pid = os.getpid()
        sampled = []
        for proc in psutil.process_iter(["pid", "name"]):
            try:
                ...
                info = proc.info
                sampled.append((proc, info.get("pid"), info.get("name") or "?"))
            except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
                continue
        time.sleep(0.4)
        ncpu = psutil.cpu_count() or 1
        groups: dict[str, dict] = {}
        for proc, pid, name in sampled:
            try:
                pct = proc.cpu_percent(None)  # core-relative % over the window
            except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
                continue  # died during the sampling window
            g = groups.setdefault(name, {"name": name, "count": 0, "pct": 0.0, "is_self": False})
            g["count"] += 1
            g["pct"] += pct
            if pid == self_pid:
                g["is_self"] = True
        ranked = sorted(groups.values(), key=lambda g: g["pct"], reverse=True)
        top, rest = ranked[:10], ranked[10:]
        return {
            "processes": [
                {
                    "name": g["name"],
                    "count": g["count"],
                    "cpu_percent": round(g["pct"] / ncpu, 1),
                    "is_self": g["is_self"],
                }
                for g in top
            ],
            "more_count": len(rest),
            "more_percent": round(sum(g["pct"] for g in rest) / ncpu, 1),
...
@router.get("/api/cpu/processes")
async def cpu_processes():
    return await asyncio.to_thread(_cpu_processes)
...
async def airllm_download_start(body: dict):
    ...
    asyncio.create_task(_run_download(model), name="model_download")
    return {"ok": True, "model": model}
...
async def post_observer_enable():
    ...
    if not obs.is_running():
        from core.watchdog import clear_state
        clear_state("observer")
        asyncio.create_task(obs.start(), name="observer_start")
...
async def post_observer_disable():
    ...
    obs = obs_mod.get_observer()
    if obs.is_running():
        asyncio.create_task(obs.stop(), name="observer_stop")
...
async def post_proactive_enable():
    ...
    if not pro.is_running():
        from core.watchdog import clear_state
        clear_state("proactive")
        asyncio.create_task(pro.start(), name="proactive_start")
...
async def post_proactive_disable():
    ...
    pro = pro_mod.get_proactive()
    if pro.is_running():
        asyncio.create_task(pro.stop(), name="proactive_stop")
...
@router.get("/api/research/sites")
async def list_research_sites_endpoint():
...
async def serve_clip(filename: str):
    ...
    return FileResponse(str(p), media_type=_AUDIO_MIME.get(p.suffix.lower(), "application/octet-stream"))
...
async def system_health():
    ...
    async def guarded(name: str, coro):
        # A probe that raises mid-body (e.g. connection refused) must still
        # produce a row — otherwise the service disappears from the health
        # panel exactly when it is down.
        try:
            await coro
        except Exception as exc:
    ...
    await asyncio.gather(
        guarded("ollama", probe_ollama()),
        guarded("hass", probe_hass()),
        guarded("chromadb", probe_chromadb()),
        guarded("gcal", probe_gcal()),
        guarded("email", probe_email()),
        guarded("memory", probe_memory()),
        guarded("fallback_llm", probe_fallback_llm()),
    )
...
async def run_mcp_health_monitor() -> None:
    while True:
        await _watchdog.watchdog_gate("mcp_health_monitor")
        try:
...
async def add_scheduled_message(body: dict):
    ...
    warn = _watchdog.reactivation_warning("scheduled_msg_loop", "A scheduled message was created")
    return {"ok": True, "id": msg_id, "fire_at": fire_at, "warning": warn or None}
...
async def run_resource_alert_loop() -> None:
    while True:
        await _watchdog.watchdog_gate("resource_alert_loop")
        try:
...
# Back-compat shims — core/main.py and tests import these names from here.
def register_watchdog_task(name: str, factory, description: str = "") -> None:
    _watchdog.register(name, factory, description)


def _watchdog_set_task(name: str, task) -> None:
    _watchdog.set_task(name, task)
...
async def get_watchdog():
    ...
    for t in sorted(all_tasks, key=lambda t: t.get_name()):
        coro = t.get_coro()
        qual = (getattr(coro, "__qualname__", "") or type(coro).__name__) if coro else ""
        task_list.append({"name": t.get_name(), "done": t.done(), "coro": qual})
    return {"tasks": task_list, "total": len(all_tasks), "named": _watchdog.status()}


async def _watchdog_action(name: str, action: str) -> dict:
    try:
        if action == "pause":
            await _watchdog.pause(name)
        elif action == "resume":
            await _watchdog.resume(name)
        elif action == "stop":
            await _watchdog.stop(name)
        elif action == "restart":
            await _watchdog.restart(name)
    except KeyError:
        ...
        raise HTTPException(status_code=500, detail=str(exc))
    return {"ok": True, "name": name, "state": _watchdog.get_entry(name).state}


@router.post("/api/watchdog/pause/{name}")
async def pause_watchdog_task(name: str):
    return await _watchdog_action(name, "pause")


@router.post("/api/watchdog/resume/{name}")
async def resume_watchdog_task(name: str):
    return await _watchdog_action(name, "resume")


@router.post("/api/watchdog/stop/{name}")
async def stop_watchdog_task(name: str):
    return await _watchdog_action(name, "stop")


@router.post("/api/watchdog/restart/{name}")
async def restart_watchdog_task(name: str):
    return await _watchdog_action(name, "restart")


@router.post("/api/watchdog/reactivate/respond/{pending_id}")
async def watchdog_reactivate_respond(pending_id: str, body: dict):
    name = _watchdog.respond_reactivation(pending_id)
    if name is None:
        raise HTTPException(status_code=404, detail="Unknown or expired reactivation request")
    if bool(body.get("approved", False)):
        await _watchdog.reactivate(name)
        return {"ok": True, "name": name, "state": _watchdog.get_entry(name).state}
    return {"ok": True, "name": name, "state": _watchdog.get_entry(name).state, "denied": True}
...
async def _docker_action(container_id: str, action: str) -> dict:
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="Docker is not installed on this system")
    _, stderr = await proc.communicate()
    if proc.returncode != 0:
